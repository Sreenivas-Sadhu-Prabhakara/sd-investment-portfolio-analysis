...
| **Asset Type** | Portfolio Analysis |
| **Control Record** | Portfolio Analysis Analysis |
...
| POST | `/v1/portfolio-analysis-analysis/initiate` | Initiate |
| GET | `/v1/portfolio-analysis-analysis` | Retrieve (list) |
| GET | `/v1/portfolio-analysis-analysis/{crId}/retrieve` | Retrieve |
| PUT | `/v1/portfolio-analysis-analysis/{crId}/update` | Update |
| PUT | `/v1/portfolio-analysis-analysis/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/portfolio-analysis-analysis/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-investment-portfolio-analysis:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-investment-portfolio-analysis ./helm -n bian-operations
```

Exposed through the platform Gateway at path prefix `/sd-investment-portfolio-analysis` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
